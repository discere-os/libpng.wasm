/**
 * libpng.wasm - Dynamic linking PNG decoder with dlopen() zlib loading
 * Production-ready WASM implementation with proper dependency architecture
 * 
...
// Dynamic zlib function pointers
static void* zlib_handle = NULL;
static int (*compress_fn)(unsigned char*, unsigned long*, const unsigned char*, unsigned long) = NULL;
static int (*uncompress_fn)(unsigned char*, unsigned long*, const unsigned char*, unsigned long) = NULL;
static unsigned long (*compressBound_fn)(unsigned long) = NULL;
...
static int load_zlib_library(void);
...
/**
 * Initialize libpng.wasm with dynamic zlib loading
 * Returns 1 on success, 0 on failure
 */
EMSCRIPTEN_KEEPALIVE
int png_wasm_init(void) {
    // Load zlib.wasm as SIDE_MODULE via dlopen
    if (load_zlib_library() != 0) {
        printf("Warning: zlib.wasm dynamic loading failed, using built-in compression\n");
        // Continue without dynamic zlib - PNG will use internal compression
    } else {
        printf("✅ zlib.wasm loaded successfully via dlopen\n");
    }
...
/**
 * Load zlib.wasm SIDE_MODULE dynamically
 */
static int load_zlib_library(void) {
    // Try to load zlib.wasm SIDE_MODULE
    ...
    // Load required zlib functions
    compress_fn = (int(*)(unsigned char*, unsigned long*, const unsigned char*, unsigned long))
                  dlsym(zlib_handle, "compress");
    uncompress_fn = (int(*)(unsigned char*, unsigned long*, const unsigned char*, unsigned long))
                    dlsym(zlib_handle, "uncompress");
    compressBound_fn = (unsigned long(*)(unsigned long))
                       dlsym(zlib_handle, "compressBound");
    
    if (!compress_fn || !uncompress_fn || !compressBound_fn) {
        printf("Failed to load zlib symbols: %s\n", dlerror());
        dlclose(zlib_handle);
        zlib_handle = NULL;
        return -1;
    }
...
/**
 * Decode PNG from buffer with dynamic zlib integration
 */
EMSCRIPTEN_KEEPALIVE
int png_wasm_decode_buffer(
    ...
    png_set_read_fn(png, &read_state, png_wasm_read_callback);
...
int png_wasm_encode_buffer(
    ...
    png_set_write_fn(png, &write_state, png_wasm_write_callback, NULL);
...
int png_wasm_get_simd_supported(void) {
    return 1; // WASM SIMD always available in our build
}
